...
/*   Updated: 2021/12/06 05:52:43 by iyoshiha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void creat_oneline(t_txt *txt, char **save)
{
	...
	i = 0;
	len_after_breakline = txt->save_len - txt->newline_index;
	if (txt->newline_index == GNL_NEWLINE_NOT_FOUND)
		txt->line = (char *)malloc(txt->save_len + END_STR);
	else
		txt->line = (char *)malloc(NEXT_INDEX_OF txt->newline_index + END_STR); // 1 for null, 1 for conv index to len;
	while ((*save)[i] != '\0')
	{
		txt->line[i] = (*save)[i];
		i = NEXT_INDEX_OF i;
	...
	txt->line[i] = '\0';
	if ((txt->len_read == END_OF_FILE) && (txt->newline_index == GNL_NEWLINE_NOT_FOUND)) //last line // find_newline's return value can be flag
	{
		free(*save);
		*save = NULL;
		return;
	...
	*save = (char *)malloc((len_after_breakline));
	ft_strlcat(*save, (old_save + NEXT_INDEX_OF txt->newline_index), len_after_breakline);
	free(old_save);
}
...
char *get_next_line(int fd)
{
	...
	find_newline(save, &txt); // 1
	while (UNTIL_REACH_EOF_OR_FOUND_NEWLINE) // 1
	{
		txt.len_read = read(fd, buf, BUFFER_SIZE);
		// printf("\\n : %d\nlen_red : %zd\nsave_len : %d\n\n", txt.newline_index, txt.len_read, txt.save_len); // dele
